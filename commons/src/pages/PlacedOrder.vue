  ...
  <div>
    <v-toolbar app color="primary" dark :extended="extended">
      ...
      <v-toolbar-title v-if="$store.state.showToolbarTitles"
        >Order ID {{ order.id }}</v-toolbar-title
      >
      <v-text-field
        label="Search basket..."
        clearable
      ...
      <v-btn
        icon
        @click.stop="deleteConfirmationDialog = true"
        v-if="
          (user.type === 'Reseller' && order.status === 'placed') ||
            (user.type === 'Reseller' && order.status === 'in progress')
        "
      >
      ...
      <div>
        <div class="px-3 pt-1 mb-2">
          <v-layout row wrap>
              ...
              <div class="text-xs-left" style="font-size: 11px;">
                <v-avatar size="20px" v-if="!order.isOffline">
                  <v-img
                    ...
                    contain
                    v-if="
                      !order.offlineContact.downloadURL &&
                        order.offlineContact.gender === 'Male'
                    "
                  ></v-img>
                  <v-img
                    ...
                    v-else-if="
                      !order.offlineContact.downloadURL &&
                        order.offlineContact.gender === 'Female'
                    "
                  ></v-img>
                  <v-img
                    :src="order.offlineContact.downloadURL"
                    contain
                    v-else-if="order.offlineContact.downloadURL"
                  ></v-img>
                </v-avatar>
                <v-avatar size="20px" v-else>
                  <img v-lazy="order.offlineContact.imageObj" /> </v-avatar
                >&nbsp;
                <span v-if="!order.isOffline"
                  >{{ order.offlineContact.firstName }}
                  {{ order.offlineContact.middleInitial || "" }}
                  {{ order.offlineContact.lastName }}</span
                >
                <span v-else
                  >{{ order.offlineContact.firstName }}
                  {{ order.offlineContact.middleInitial || "" }}
            ...
            <v-flex xs3>
              <div class="text-xs-right" style="font-size: 11px;">
          ...
          <tbody>
            <tr
              v-for="(item, i) in filterBy(
                orderBy(order.basket, 'created_at', -1),
                search
              )"
              :key="i"
            >
            ...
            <tr>
              <td>
                ...
                <v-btn
                  ...
                  flat
                  color="primary"
                  @click="openSheet('discount')"
                  style="height: 50%; font-size: 12px;"
                >
            ...
            <tr class="border-top">
              <td>
                <v-btn
                  ...
                  flat
                  color="primary"
                  @click="openSheet('delivery_charge')"
                  style="height: 50%; font-size: 12px;"
                >
                  <v-icon small>border_color</v-icon>
                </v-btn>
            ...
            <tr class="border-top">
              <td>
                <v-btn
                  ...
                  flat
                  color="primary"
                  @click="openSheet('payment_due_date')"
                  style="height: 50%; font-size: 12px;"
                >
                  <v-icon small>border_color</v-icon>
                </v-btn>
            ...
            <v-expansion-panel-content>
              ...
              <v-card>
                ...
                <v-card-text class="py-2">
                  <p class="text-xs-right body-2">
                    Total Outstanding: {{ totalOutstanding | currency("P ") }}
                  </p>
                </v-card-text>
              </v-card>
            </v-expansion-panel-content>
                ...
                <v-card-text class="py-2">
                  <div v-if="order.proposed_delivery_schedule">
                    <v-layout row wrap>
                      ...
                      <v-flex xs9>{{
                        order.proposed_delivery_schedule.date
                      }}</v-flex>
                      <v-flex xs3>Time</v-flex>
                      <v-flex xs9>{{
                        order.proposed_delivery_schedule.time
                      }}</v-flex>
                      <v-flex xs3>Location</v-flex>
                      <v-flex xs9>{{
                        order.proposed_delivery_schedule.location
                      }}</v-flex>
                      <v-flex
                        xs3
                        v-if="order.proposed_delivery_schedule.remarks"
                        >Remarks</v-flex
                      >
                      <v-flex
                        xs9
                        v-if="order.proposed_delivery_schedule.remarks"
                        >{{ order.proposed_delivery_schedule.remarks }}</v-flex
                      >
                    </v-layout>
                    <div class="text-xs-center">
                      <v-btn
                        ...
                        depressed
                        color="primary"
                        @click="openSheet('schedule_delivery')"
                        >Edit</v-btn
                      >
                    </div>
                  </div>
        ...
        <div v-if="order.status == 'delivered'" class="text-xs-center mt-2">
          <h3>
            Delivered on {{ order.delivered_date | momentify("DD-MMM-YYYY") }}
          </h3>
        </div>
        <v-bottom-sheet v-model="sheet">
            ...
            <v-card-text class="py-0">
              ...
              <div class="pa-2">
                <v-btn
                  block
                  class="primary white--text"
                  @click.native="addDeliveryCharge"
                  :disabled="disableButton"
                  :loading="disableButton"
                  >Add Delivery Charge</v-btn
                >
              </div>
            </v-card-text>
          </v-card>
          <v-card v-else-if="selected === 'payment_due_date'">
            ...
            <v-card-text class="py-0">
              <v-text-field
                ...
                v-model="payment_due_date.value"
                @click="openCalendar"
                readonly
              ></v-text-field>
              <div class="pa-2">
                <v-btn
                  ...
                  :disabled="disableButton"
                  :loading="disableButton"
                  @click.native="addPaymentDueDate"
                  >Add Payment Due Date</v-btn
                >
              </div>
            </v-card-text>
          </v-card>
          <v-card v-else-if="selected === 'record_payment'">
            ...
            <v-card-text class="py-0">
              <v-text-field
                placeholder="Enter amount here..."
                v-model="record_payment.value"
                type="number"
              ></v-text-field>
              <v-text-field
                readonly
                placeholder="Payment made on"
                v-model="record_payment.date"
                @click="openCalendar"
              ></v-text-field>
              <div class="pa-2">
                <v-btn
                  block
                  :disabled="recordPaymentIsLoading"
                  :loading="recordPaymentIsLoading"
                  class="primary white--text"
            ...
            <v-card-text class="py-0">
              <div class="mb-3" v-if="order.delivery_schedule_status">
                <v-chip
                  color="green"
                  text-color="white"
                  v-if="order.delivery_schedule_status === 'accepted'"
                >
              ...
              <v-text-field
                ...
                v-model="schedule_delivery.time"
              ></v-text-field>
              <v-text-field
                label="Location"
                v-model="schedule_delivery.location"
              ></v-text-field>
              <v-text-field
                label="Remarks"
                v-model="schedule_delivery.remarks"
              ></v-text-field>
              <div class="pa-2">
                <v-btn
                  ...
                  block
                  class="primary white--text"
                  @click.native="saveToCalendar"
                  >Save to Calendar</v-btn
                >
                <v-btn
                  v-else
                  block
                  class="primary white--text"
                  @click.native="proposeDeliverySchedule"
                  >Send Proposed Delivery Date</v-btn
                >
              </div>
            </v-card-text>
          </v-card>
    ...
    <v-dialog v-model="deliveredDialog" width="250">
      <v-card>
        <v-card-text>
          <div
            class="headline text-xs-center blue--text text--darken-2"
            v-if="order.isOffline"
          >
            Order No. {{ order.id }} has been received by customer
            {{ order.offlineContact.firstName }}
            {{ order.offlineContact.middleInitial || "" }}
            {{ order.offlineContact.lastName || "" }}. Your inventory has been
    ...
    <v-dialog v-model="deleteConfirmationDialog">
      <v-card>
        ...
        <v-card-text
          >This will permanently remove the order in the database.</v-card-text
        >
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
            ...
            >No</v-btn
          >
          <v-btn color="red" flat @click="deleteOrder">Yes</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <ConfirmationModal
      ...
      confirmText="Yes"
      @confirmClicked="markAsDeliveredConfirm"
    />
    <Modal ref="Modal" />
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    if (this.order.proposed_delivery_schedule) {
      this.schedule_delivery = Object.assign(
        {},
        this.order.proposed_delivery_schedule
      );
    }
  ...
  methods: {
    ...
    openTimePicker() {
      const options = {
        ...
        success: newTime => {
          this.schedule_delivery.time = this.$moment(newTime).format("hh:mm A");
        }
      };
      ...
      const options = {
        ...
        success: newDate => {
          if (this.selected === "payment_due_date") {
            this.payment_due_date.value = this.$moment(newDate).format(
              "MM/DD/YYYY"
            );
          } else if (this.selected === "record_payment") {
            this.record_payment.date = this.$moment(newDate).format(
              "MM/DD/YYYY"
            );
          } else if (this.selected === "schedule_delivery") {
            this.schedule_delivery.date = this.$moment(newDate).format(
              "MM/DD/YYYY"
            );
          }
        }
    ...
    recordPayment() {
      ...
      if (!this.order.hasOwnProperty("payments")) {
        ...
        if (this.payment_ref) {
          const i = data.object.payments.findIndex(
            p => p.ref == this.payment_ref
          );
          if (i >= 0) {
    ...
    proposeDeliverySchedule() {
      if (
        !this.schedule_delivery.date ||
        !this.schedule_delivery.time ||
        !this.schedule_delivery.location
      ) {
        return;
      }

      const scheduleDelivery = JSON.parse(
        JSON.stringify(this.schedule_delivery)
      );
      scheduleDelivery.timestamp = +this.$moment(
    ...
    updatePlacedOrder(data) {
      ...
      this.$store
        .dispatch("orders/UPDATE_ORDER", data)
        .then(() => {
          if (data.object.status != "delivered") {
    ...
    async markAsDeliveredConfirm() {
      ...
      if (items.length > 0) {
        ...
        this.$refs.Modal.show(
          "Error",
          "Items with less inventory than orders found. Please update your inventory."
        );
        this.disableButton = false;
        return;
      }
      ...
      await this.$store.dispatch(
        "inventory/UPDATE_INVENTORY_NET",
        this.order.id
      );
    ...
    deleteOrder() {
      console.log(this.order);
      this.$store
        .dispatch("orders/DELETE_ORDER", this.order.id)
        .then(() => {
          this.$router.go(-1);
        })
        .catch(error => {
  ...
  computed: {
    ...
    totalOutstanding() {
      ...
      if (this.order.payments) {
        totalPayments = this.order.payments.reduce(function(a, b) {
          return +a + +b.value;
        }, 0);
      }
      if (this.order.delivery_charge) {
