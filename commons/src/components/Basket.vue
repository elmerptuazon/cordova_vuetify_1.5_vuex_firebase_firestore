  ...
  <div>
    <v-toolbar app color="primary" dark :extended="extended">
      ...
      <div v-if="$store.state.showToolbarTitles">
        <v-toolbar-title v-if="user.type === 'Customer'">Orders</v-toolbar-title>
        <v-toolbar-title v-else-if="user.type === 'Reseller'">Your Orders</v-toolbar-title>
      </div>
      <v-text-field v-model="search" label="Search order..." clearable v-if="extended" slot="extension" class="mx-3" flat solo-inverted prepend-icon="search"></v-text-field>
      <v-spacer></v-spacer>
      ...
      <template slot="items" slot-scope="props">
        <tr @click="viewOrder(props.item.orderNo)">
          <td class="pa-2">
            ...
            <span class="caption">Status: {{ props.item.status | status }}</span>
          </td>
          <td class="text-xs-center body-1">{{ props.item.created_at | momentify('DD-MMM-YYYY')}}</td>
          <td class="text-xs-center body-1">{{ props.item.total | currency('P')}}</td>            
        </tr>
      </template>
    </v-data-table>
  </v-container>
