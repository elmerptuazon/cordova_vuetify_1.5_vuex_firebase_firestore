<template>
	<v-container fluid grid-list-md class="pa-4">
		<v-form v-model="valid" ref="form" lazy-validation @submit.prevent="submit">
			<v-layout row wrap>
				<v-flex xs12>
					<v-text-field :rules="basicRules" required label="First Name" v-model="registerData.firstName"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-text-field label="Middle Name" v-model="registerData.middleInitial"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-text-field :rules="basicRules" required label="Last Name" v-model="registerData.lastName"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-text-field label="Birthday" append-icon="date_range" required :rules="basicRules" @click="openCalendar" v-model="registerData.birthday"></v-text-field>
				</v-flex>
				<v-flex xs12>
				...
				<v-flex xs12>
					<v-text-field :rules="mobileRules" type="number" append-icon="contact_phone" required label="Mobile number" v-model="registerData.contact"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-text-field :rules="basicRules" required label="House" v-model="registerData.address.house"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-text-field :rules="basicRules" required label="Street Name" v-model="registerData.address.streetName"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-autocomplete :rules="basicRules" required :items="provinces" item-value="name" label="Province" item-text="name" v-model="registerData.address.province"></v-autocomplete>
				</v-flex>
				<v-flex xs12>
					<v-autocomplete :rules="basicRules" required v-if="registerData.address.province" item-value="name" :items="cities" label="City/Municipality" item-text="name" v-model="registerData.address.citymun"></v-autocomplete>
				</v-flex>
				<v-flex xs12>
					<v-text-field :rules="basicRules" required label="Zip Code" v-model="registerData.address.zipCode"></v-text-field>
				</v-flex>
				<v-flex xs12>
					<v-text-field :rules="basicRules" required label="Tax Identification Number (TIN)" v-model="registerData.tin"></v-text-field>
				</v-flex>
			</v-layout>
			...
			<div class="px-4 mt-4">
				...
				<v-btn flat color="grey darken-1" dark v-show="registerData.displayPicture" despressed block @click="!registerData.displayPicture">Remove Profile Picture</v-btn>
				...
				<v-btn color="grey darken-1" dark flat v-show="registerData.proofOfId" despressed block @click="!registerData.proofOfId">Remove Proof of ID</v-btn>
				
				<v-btn :disabled="submitBtnDisabled" :loading="submitBtnDisabled" depressed class="primary white--text" block large type="submit">Submit</v-btn>
			</div>
...
export default {
	...
	methods: {
		async submit () {
			...
			registerData.firstName = this.capitalizeFirstLetter(registerData.firstName);
			registerData.lastName = this.capitalizeFirstLetter(registerData.lastName);
			registerData.middleInitial = this.capitalizeFirstLetter(registerData.middleInitial);
			...
			try {
				...
				const response = await this.$store.dispatch('accounts/REGISTER_DISTRIBUTOR', registerData);

				this.enableBack();
				this.submitBtnDisabled = true;

				this.$refs.modal.show('Success', 'Please wait for admin approval before you can login to your account.');
			} catch (error) {
