  ...
  <div>
    <v-toolbar app color="primary" dark :extended="extended">
      ...
      <v-toolbar-title v-if="$store.state.showToolbarTitles"
        >Order ID {{ GET_ORDER.orderNo }}</v-toolbar-title
      >
      <v-text-field
        label="Search basket..."
        clearable
          ...
          <v-layout row wrap>
            <v-flex xs4>
              <div class="text-xs-left">
                {{ GET_ORDER.created_at | momentify("DD-MMM-YYYY") }}
              </div>
            </v-flex>
            <v-flex xs8>
              <div class="text-xs-right">
                Status: {{ GET_ORDER.status | status }}
              </div>
            </v-flex>
          </v-layout>
        </div>
          ...
          <tbody>
            <tr
              v-for="(item, i) in filterBy(
                orderBy(GET_ORDER.basket, 'created_at', -1),
                search
              )"
              :key="i"
            >
            ...
            <tr>
              ...
              <td class="caption text-xs-right">
                <div v-if="GET_ORDER.discount">
                  ...
                  <span v-else> {{ GET_ORDER.discount.value }}% </span>
                </div>
                <div v-else>
                  0
                </div>
        ...
        <v-card v-if="GET_ORDER.proposed_delivery_schedule">
          ...
          <v-card-text>
            <v-layout row wrap>
              ...
              <v-flex xs9>{{
                GET_ORDER.proposed_delivery_schedule.date
              }}</v-flex>
              <v-flex xs3>Time</v-flex>
              <v-flex xs9>{{
                GET_ORDER.proposed_delivery_schedule.time
              }}</v-flex>
              <v-flex xs3>Location</v-flex>
              <v-flex xs9>{{
                GET_ORDER.proposed_delivery_schedule.location
              }}</v-flex>
              <v-flex xs3 v-if="GET_ORDER.proposed_delivery_schedule.remarks"
                >Remarks</v-flex
              >
              <v-flex xs9 v-if="GET_ORDER.proposed_delivery_schedule.remarks">{{
                GET_ORDER.proposed_delivery_schedule.remarks
              }}</v-flex>
            </v-layout>
          </v-card-text>
        </v-card>
    ...
    <v-bottom-sheet v-model="deliveryScheduleSheet" persistent>
      <v-card v-if="GET_ORDER.proposed_delivery_schedule">
        ...
        <v-card-text class="py-0 subheading">
          Agent Seller has scheduled your item(s) delivery date. Do you agree
          with the delivery date and time?
          <div class="mt-3"></div>
          <v-layout row wrap>
            <v-flex xs3>Date</v-flex>
              ...
              <v-layout row wrap>
                <v-flex xs6>
                  <v-btn
                    ...
                    @click.native="updateDeliverySchedule('accepted')"
                    >Accept</v-btn
                  >
                </v-flex>
                <v-flex xs6>
                  <v-btn
                    ...
                    :disabled="disableButton"
                    :loading="disableButton"
                    @click.native="updateDeliverySchedule('declined')"
                    >Decline</v-btn
                  >
                </v-flex>
              </v-layout>
  ...
  created() {
    this.unsubscribeToOrders = COLLECTION.orders
      ...
      .onSnapshot(doc => {
        ...
        if (
          !obj.data.delivery_schedule_status &&
          obj.data.proposed_delivery_schedule
        ) {
    ...
    updateDeliverySchedule(status) {
      ...
      this.$store
        ...
        .then(() => {
          const message =
            status == "accepted"
              ? "Proposed delivery schedule accepted"
              : "Proposed delivery schedule declined";
  ...
  computed: {
    ...
    totalOutstanding() {
      ...
      if (this.GET_ORDER.payments) {
        totalPayments = this.GET_ORDER.payments.reduce(function(a, b) {
          return +a + +b.value;
        }, 0);
      }
      if (this.GET_ORDER.delivery_charge) {
