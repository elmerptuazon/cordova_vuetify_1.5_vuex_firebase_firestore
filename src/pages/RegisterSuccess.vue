<template>
	<v-container>
		...
		<div id="reseller-container" class="mt-5 text-xs-center" v-if="userType === 'Reseller'">
			<p class="headline blue--text text--darken-2">Your application has been submitted!</p>
			<v-avatar :tile="true" size="92px" class="grey lighten-4" v-if="registerData.imageObj.src">
				<v-img class="mt-2 elevation-1 flipped" :src="registerData.imageObj.src"></v-img>
			</v-avatar>
			<p class="grey--text text--darken-2 mt-4 subheading text-xs-center">{{fullName}}</p>
			<CheckMark />
			<v-btn flat color="primary" class="mt-5" @click="goToTabs">Continue</v-btn>
		</div>

		<div id="customer-container" class="mt-2" v-else-if="userType === 'Customer'">
			<v-form v-model="resellerValid" ref="resellerForm" lazy-validation @submit.prevent="findReseller">
				...
				<div class="text-xs-center">
					<v-avatar :tile="true" size="92px" class="grey lighten-4">
						<v-img class="elevation-1 flipped" :src="resellerDetails.downloadURL"></v-img>
					</v-avatar>
				</div>
				<div class="mt-5 px-5">
					<v-btn depressed type="submit" color="primary" block large v-show="!resellerFound">Find Your Reseller</v-btn>
					<v-btn depressed type="button" color="primary" block large v-show="resellerFound" @click="confirmReseller">Confirm Reseller</v-btn>
					<div class="mt-4 text-xs-center">
						<v-btn flat type="button" color="primary" @click="goToTabs">Maybe Later</v-btn>
					</div>
				</div>
			</v-form>
		</div>
...
export default {
	...
	methods: {
		findReseller() {
			if (this.$refs.resellerForm.validate()) {
				this.Indicator().open()
				this.$store.dispatch('accounts/FIND_RESELLER', {
					...
					if (!res.empty) {
						...
						this.$store.dispatch('accounts/GET_PROFILE_PICTURE', res.data.uid)
						.then((imageData) => {
							if (imageData.success) {
								this.imgObj.src = imageData.url
							} else {
								this.imgObj.src = res.data.gender === 'Male' ? defaultMaleImage : defaultFemaleImage;
							}
		...
		confirmReseller() {
			this.$store.dispatch('accounts/ADD_CUSTOMER_TO_RESELLER', {
	...
	computed: {
		fullName () {
			return `${this.registerData.firstName} ${this.registerData.middleInitial || ''} ${this.registerData.lastName}`;
		}
	},
	watch: {
		resellerSearch () {
