<template>
	<div>
		<v-btn icon @click="goToBasket" v-show="$store.getters['accounts/isApproved']" v-if="user.type ==='Reseller'">
			<v-badge right color="red lighten-1" overlap v-model="showBasketCount">
				<span slot="badge">{{basketCount}}</span>
				<v-icon>shopping_cart</v-icon>
			</v-badge>
		</v-btn>
		<v-btn icon @click="goToBasket" v-show="$store.getters['accounts/isApproved']" v-else-if="user.type === 'Customer'">
