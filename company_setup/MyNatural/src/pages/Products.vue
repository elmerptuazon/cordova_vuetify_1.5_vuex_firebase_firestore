<template>
  <div>
    ...
    <v-container fluid>
      ...
      <div
        class="text-xs-center grey--text text--darken-2 body-2"
        v-show="noItemsMessage"
      >
      ...
      <masonry :cols="2" :gutter="8" v-if="!search">
        <v-card
          class="mb-2"
          v-for="product in orderBy(GET_PRODUCTS, 'position')"
          :key="product.id"
      ...
      <masonry :cols="2" :gutter="8" v-else>
        <v-card
          class="mb-2"
          v-for="product in filterBy(GET_PRODUCTS, search)"
          :key="product.id"
          @click="goToProduct(product)"
        >
          <div>
...
export default {
  ...
  methods: {
    ...
    goToProduct(product) {
      this.$router.push({
        name: "Item",
        params: { id: product.id, product, category: this.$route.params.id }
      });
    },
    goBack() {
      this.$router.go(-1);
  ...
  created() {
    ...
    if (this.$route.params.id) {
      this.$store
        .dispatch("products/GET_PRODUCTS", this.$route.params.id)
        .then(data => {
          this.loader = false;
          this.items = data;
