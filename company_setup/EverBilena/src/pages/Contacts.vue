<template>
	<div>
		...
		<v-toolbar app color="primary" dark :extended="extended">
			...
			<v-btn v-show="contactType === 'Offline'" icon @click="$router.push({name: 'AddOfflineContact'})">
		...
		<v-container fluid grid-list-lg class="pa-0 pt-3">
			<div class="text-xs-center">
				<!-- <v-btn-toggle v-model="contactType">
						...
						<v-list-tile avatar v-else :key="c.id" @click="viewContact(c)">
							<v-list-tile-avatar>
								<v-avatar size="50px" tile>
									<v-img :src="c.imageObj" :alt="c.firstName" class="flipped" contain></v-img>
								</v-avatar>
							</v-list-tile-avatar>
							<v-list-tile-content>
								<v-list-tile-title v-if="settings.contactsSortBy === 'first'">{{ c.firstName }} {{ c.lastName || '' }}</v-list-tile-title>
								<v-list-tile-title v-else>{{ c.lastName || '' }}, {{ c.firstName }}</v-list-tile-title>
...
export default {
	...
	created() {
		...
		// this.$store.dispatch('contacts/GET_ONLINE_CONTACTS')
		// .then(() => {
	...
	methods: {
		...
		viewContact (contact) {
			this.$router.push({name: 'ViewContact', params: { id: contact.id, user: contact }})
		},
		...
		GET_OFFLINE_LIST () {
			...
			if (this.settings.contactsSortBy == 'first') {
				...
				contacts = contacts.sort((a, b) => a.lastName.localeCompare(b.lastName))
			}
			var currentHeader
			// ADD HEADINGS
