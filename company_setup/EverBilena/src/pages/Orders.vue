<template>
	<div>
		...
		<div v-else-if="tab === 'tab2'" class="tab-content white" :style="{ height: height + 'px' }">
			<div class="py-2">
				<OrdersToEverBilena :items="items" :search="search" ref="OrdersToEverBilena" />
			</div>
		</div>
	...
	methods: {
		async change (page) {
			...
			if (page === 'tab1') {
				...
				const orders = (await this.$store.dispatch('orders/GET_ORDERS')).map((order) => {
