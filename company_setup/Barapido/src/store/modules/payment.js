...
const payment = {
    ...
    actions: {
        ...
        async RecordPaymentToLogs({}, payload) {
            try {
                const response = await DB.collection('providers').doc('magpie').collection('paymentLogs').add(payload);
                console.log('payment log recorded! ', response);
            } catch(error) {
                console.log('payment log not recorded! ', error);
            }
        },
